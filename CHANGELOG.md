...
- Updated both README files with the current Rust test count: 78 total (72 unit and 6 integration).
- TypeScript `u64` and `i64` values now use `bigint` end-to-end, preserving all 64-bit wire values beyond `2^53`.
...
- Added an optional-aware serde helper for guarded fixed-byte aliases such as `Option<Token>`.
