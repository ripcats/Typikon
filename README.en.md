...
[![Tests](https://img.shields.io/badge/Tests-97%20Passing-3FB950?style=for-the-badge&logo=githubactions&logoColor=white)](#what-is-actually-tested)
...
For local optional values, Typikon provides `Optional<T>`:
...
`Optional<T>` is encoded locally before its value: `0` means `None`, `1` means `Some(T)`, followed by the normal wire representation of `T`. It composes inside `Vec` and `Map`; `Optional<Optional<T>>` is rejected. Use `flags` and `#[guard]` when bit-level compactness or named conditions are required.

Constraints can be attached to a type alias and then apply to every field using that alias:

```rust
type SealedSession = Vec<u8> #[exact_len(49)];
...
`#[exact_len(49)]` validates the payload after normal length-prefixed `Vec<u8>` wire encoding; it is not `bytes[49]`. The constraint is included in the expanded public schema, fingerprint/C-ID, and compatibility checks.

Comments from the source schema are omitted from the public schema by default. To copy them explicitly, use `--preserve-comments`:
...
```bash
typikon generate public schema.typ --preserve-comments
```

Without the flag, the generator emits a short warning in English.

### Declarative functions

A source schema may contain a separate `functions` block. Each function declares a canonical method name, request type, and result type. `Empty` is a built-in type with a zero-byte payload.
...
Functions are not part of ordinary wire payloads and do not change structure C-IDs or fingerprints. `#[deprecated("...")]` marks a field or function as supported but discouraged for new code; SDKs can use the message as a migration hint. Functions and attributes are copied to the public schema in the same order.

As with struct fields, the trailing `;` on the last function before `}` is optional.

Generators also emit typed API facades: Rust gets `methods::Api`, Go gets `NewAPI(call)`, TypeScript gets `createApi(call)`, and Python gets `create_api(call)`. The facade takes the schema method constant, encodes the request, and decodes the typed result; the transport callback is the only network integration point.
...
The Rust suite contains **97 tests: 91 unit tests and 6 integration tests** covering parsing and semantic validation, code generation, the CLI, Layer/C-ID handling, wire round-trips, fixed byte arrays and exact-length checks, alias constraints, comments, functions, limits, malformed input, maps, VarInt, optional markers, borrowed views, language-view generation, vectored writes, randomized inputs, and FlatBuffers round-trip comparison.
