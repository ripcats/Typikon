...
The generated module exposes direct PyO3 encode/decode, `validate_borrowed_<type>`, `borrowed_<type>`, and `borrowed_packet_<type>` functions for every schema item. `BorrowedPacket` owns the packet-backed `memoryview` and its type name; Python strings, lists, and dictionaries remain materialized because of Python object ownership semantics, so no typed borrowed field API is promised. JSON serialization is not used at the Python/Rust boundary.
