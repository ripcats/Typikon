...
fn main() {
    ...
    let schema = typikon::parse_schema(&source).expect("parse schema");
    let bridge = typikon::generate_bridge(
        &schema,
        &artifacts.rust_file_name,
        typikon::BridgeKind::Python,
    );
    fs::write(out_dir.join("python-bridge.rs"), bridge).expect("write python-bridge.rs");
}
