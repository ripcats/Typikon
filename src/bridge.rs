...
pub fn generate_c_header(schema: &Schema) -> String {
    ...
    for item in &schema.items {
        ...
        output.push_str(&format!(
            "TypikonBridgeResult typikon_{}_{}_encode_json(const uint8_t *input, size_t len);\nTypikonBridgeResult typikon_{}_{}_decode_json(const uint8_t *input, size_t len);\n",
...
pub fn generate_python_binding(schema: &Schema) -> String {
    ...
    output.push_str(
        &schema
            ...
            .map(|item| {
                ...
                format!(
                    "encode_{function_name} as _native_encode_{function_name}, decode_{function_name} as _native_decode_{function_name}"
                )
            })
            .collect::<Vec<_>>()
            .join(", "),
    ...
    for item in &schema.items {
        ...
        let function_name = snake_case(name);
        output.push_str(&format!(
            ", \"{name}\", \"encode_{function_name}\", \"decode_{function_name}\""
        ));
    }
    output.push_str("]\n");
    output
...
pub fn generate_bridge(schema: &Schema, native_file: &str, kind: BridgeKind) -> String {
    ...
    if !matches!(kind, BridgeKind::Python) {
        for item in &schema.items {
            ...
            output.push_str(&format!(
            "#[unsafe(no_mangle)]\npub extern \"C\" fn typikon_{layer}_{function_name}_encode_json(input: *const u8, len: usize) -> TypikonBridgeResult {{ let bytes = match input_bytes(input, len) {{ Ok(bytes) => bytes, Err(error) => return TypikonBridgeResult::failure(error) }}; match encode_json_{function_name}(bytes) {{ Ok(data) => TypikonBridgeResult::success(data), Err(error) => TypikonBridgeResult::failure(error) }} }}\n"
        ));
            output.push_str(&format!(
            "#[unsafe(no_mangle)]\npub extern \"C\" fn typikon_{layer}_{function_name}_decode_json(input: *const u8, len: usize) -> TypikonBridgeResult {{ let bytes = match input_bytes(input, len) {{ Ok(bytes) => bytes, Err(error) => return TypikonBridgeResult::failure(error) }}; match decode_json_{function_name}(bytes) {{ Ok(data) => TypikonBridgeResult::success(data), Err(error) => TypikonBridgeResult::failure(error) }} }}\n"
        ));
        }
    }
    if matches!(kind, BridgeKind::Python) {
    ...
    fn bridges_reference_the_same_native_backend() {
        ...
        for kind in BridgeKind::ALL {
            ...
            if matches!(kind, BridgeKind::Python) {
                ...
                assert!(source.contains("encode_json_message"));
            }
        }
        assert_eq!(
