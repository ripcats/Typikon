...
pub fn generate_rust(schema: &Schema) -> String {
    ...
    output.push_str("#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]\npub struct Empty;\nimpl typikon::WireCodec for Empty { fn encode(&self, _: &mut typikon::Encoder) -> Result<(), typikon::WireError> { Ok(()) } fn decode(_: &mut typikon::Decoder<'_>) -> Result<Self, typikon::WireError> { Ok(Self) } }\n\n");
    for length in fixed_byte_lengths(schema) {
    ...
    for item in &schema.items {
        match item {
            Item::Alias(alias) => output.push_str(&format!(
                "pub type {} = {};\n",
                alias.name,
                rust_type(&alias.ty)
            )),
                ...
                if format == PublicSchemaFormat::Compact {
                        ...
                        &item
                            ...
                            .map(|field| {
                                let deprecated = field
                                    .deprecated
                                    .as_ref()
                                    .map(|message| format!("#[deprecated(\"{message}\")] "))
                                    .unwrap_or_default();
                                let guard = field
                                    .guard
                                    .as_ref()
                    ...
                    for (index, field) in item.fields.iter().enumerate() {
                        ...
                        let exact_len = field
                            ...
                            .map(|length| format!(" #[exact_len({length})]"))
                            .unwrap_or_default();
                        let separator = if index + 1 == item.fields.len() {
                            ""
                        } else {
                            ","
                        };
            ...
            Item::Enum(item) => {
                ...
                if format == PublicSchemaFormat::Compact {
                    ...
                    for (index, variant) in item.variants.iter().enumerate() {
                        let separator = if index + 1 == item.variants.len() {
                            ""
                        } else {
                            ","
                        };
        ...
        let fields = variant
            ...
            .map(|field| {
                let exact = field
                    ...
                    .unwrap_or_default();
                format!("{}: {}{}", field.name, schema_type(&field.ty), exact)
            })
            .collect::<Vec<_>>()
            .join(", ");
    ...
    for field in &item.fields {
        ...
        if let Some(guard) = &field.guard {
            ...
            let decode = if is_byte_vec(&field.ty) {
                let check = field.exact_len.map(|n| format!("if value.len() != {n} {{ return Err(typikon::WireError::MalformedConstructor); }} ")).unwrap_or_default();
                format!("{{ let value = decoder.bytes()?; {check} value }}")
            } else {
            ...
            if let Some(length) = field.exact_len {
                output.push_str(&format!(
                    "        if {}.len() != {} {{ return Err(typikon::WireError::MalformedConstructor); }}\n",
                    field.name, length
                ));
            }
...
pub(crate) fn fixed_byte_length(ty: &Type, schema: &Schema) -> Option<usize> {
    match ty {
        Type::FixedBytes(length) => Some(*length),
        Type::Primitive(name) => schema.items.iter().find_map(|item| match item {
            Item::Alias(alias) if &alias.name == name => fixed_byte_length(&alias.ty, schema),
            _ => None,
        }),
        _ => None,
    }
...
fn fixed_byte_lengths(schema: &Schema) -> BTreeSet<usize> {
    fn collect(ty: &Type, schema: &Schema, lengths: &mut BTreeSet<usize>) {
        if let Some(length) = fixed_byte_length(ty, schema) {
            lengths.insert(length);
        }
        match ty {
            Type::Vec(item) => collect(item, schema, lengths),
            Type::Map(key, value) => {
                collect(key, schema, lengths);
                collect(value, schema, lengths);
            }
            _ => {}
        }
    }
    ...
    for item in &schema.items {
        match item {
            Item::Alias(alias) => collect(&alias.ty, schema, &mut lengths),
            Item::Struct(item) => item
                .fields
                .iter()
                .for_each(|field| collect(&field.ty, schema, &mut lengths)),
            Item::Enum(item) => item
                .variants
                .iter()
                .flat_map(|variant| &variant.fields)
...
mod tests {
    ...
    #[test]
    fn generates_fixed_bytes_exact_lengths_and_aliases() {
        let schema = parse_schema(
            "#[version(1)] type ConnectionId = bytes[16]; struct Packet { id: ConnectionId, hash: Vec<u8> #[exact_len(32)], }",
        )
        .unwrap();
        let generated = generate_rust(&schema);
        assert!(generated.contains("pub type ConnectionId = [u8; 16];"));
        assert!(generated.contains("pub id: ConnectionId,"));
        assert!(generated.contains("self.hash.len() != 32"));
        assert!(generated.contains("hash.len() != 32"));
    }
    ...
    #[test]
    fn expands_data_enum_variants_in_public_schema() {
        let schema = parse_schema(
            "#[version(10)] enum Update { Created { body: String, }, Deleted { id: u64, }, }",
        )
        .unwrap();
        let public = generate_public_schema(&schema);
        let compact = generate_public_schema_compact(&schema);
        assert!(public.contains("enum Update {\n"));
        assert!(public.contains("    #[cid(") && public.contains(")] Created { body: String },"));
        assert!(public.contains(")] Deleted { id: u64 }"));
    ...
    #[test]
    fn generates_function_catalog_and_typed_helpers() {
        let schema = parse_schema(
            "#[version(4)] struct User { id: u64, } functions { user.create: Empty -> User; }",
        )
        .unwrap();
    ...
    #[test]
    fn public_functions_block_keeps_closing_brace() {
        let schema = parse_schema(
            "#[version(1)] struct RpcData { value: u8, } functions { system.ping: RpcData -> RpcData; session.resume: RpcData -> RpcData; }",
        )
        .unwrap();
        let public = generate_public_schema(&schema);
